...
class VisitorAnnouncePage {
  ...
  async announceVisitor(dni: string): Promise<void> {
    ...
    console.log('✅ Solicitando autorización del visitante...');
    // Solicitar autorización
    await this.page.getByRole('button', { name: 'Solicitar autorización' }).waitFor({ state: 'visible', timeout: this.timeout });
    await this.page.getByRole('button', { name: 'Solicitar autorización' }).click();
    await this.page.waitForTimeout(2000);
    
    // Confirmar mensaje de éxito
    await this.page.getByText('Hemos notificado a los').waitFor({ state: 'visible', timeout: this.timeout });
    ...
    console.log('✅ Solicitud de autorización completada exitosamente');
  }
}
...
test('test_announce_visitor', async ({ page }) => {
  ...
  try {
    ...
    const visitorAnnouncePage = new VisitorAnnouncePage(page);
    await visitorAnnouncePage.login(USER_EMAIL, USER_PASSWORD);
    ...
    await visitorAnnouncePage.announceVisitor(VISITOR_DNI);
